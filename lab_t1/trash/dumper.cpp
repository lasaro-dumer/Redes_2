...
// unsigned char pkgBuff[BUFFSIZE]; // buffer de recepcao
//
// int sockd;
// int on;
// struct ifreq ifr;
...
void dumper::printMac(unsigned char *ptrMac){
...
*/
enum etherpack_type dumper::getPackageType(u_int16_t tp){
...
bool dumper::cmp(pair<string,unsigned int> const & a, pair<string,unsigned int> const & b)
{
     return a.second != b.second?  a.second > b.second : a.first > b.first;
}

bool dumper::cmp2(pair<uint16_t,unsigned int> const & a, pair<uint16_t,unsigned int> const & b)
{
     return a.second != b.second?  a.second > b.second : a.first > b.first;
}

string dumper::getStringFromBuff(unsigned char* pkgBuff, int start,int* stop){
    stringstream ss;
    int next = start;
    unsigned char cntC = pkgBuff[next];
    while (cntC > 0) {
        next++;
...
string dumper::processPacket(unsigned char* pkgBuff, long buffSize, ssize_t pkgSize)
{
    cnt_TOTAL++;
    totalSize = totalSize + pkgSize;
    // if(minSize > pkgSize)
    //     minSize = pkgSize;
    // if(maxSize < pkgSize)
    //     maxSize = pkgSize;

    struct ether_header *etHdr = (struct ether_header *) pkgBuff;
    enum etherpack_type pktype = getPackageType(etHdr->ether_type);
    switch (pktype) {
        case IPv4:
        {
            //see /usr/include/netinet/ip.h
            struct ip *ipPart = (struct ip *) &pkgBuff[14];
            ...
            switch (ipPart->ip_p) {
                case 1:{
                    cnt_ICMP++;
                    struct icmphdr *icmpPart = (struct icmphdr *)&pkgBuff[p];
                ...
                case 6:{
                    cnt_TCP++;
                    struct tcphdr *tcpPart = (struct tcphdr *)&pkgBuff[p];
                    if(tcpPart->syn == 1){
                        cnt_TCP_ConnUP++;
                    }
                ...
                case 17:{
                    cnt_UDP++;
                    struct udphdr *udpPart = (struct udphdr *)&pkgBuff[p];
        ...
        case ARP:{
            ...
            cnt_ARP++;
            struct arphdr *arpPart = (struct arphdr *) &pkgBuff[14];
